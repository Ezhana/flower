...
The local generator topologically orders nominal declarations and lowers oversized parameter tuples into callee memory. `sdk/go/internal/componentabi/generate.go` invokes it directly. Generated bindings are a single deterministic output; no post-generation mutation is permitted. `just bindings` regenerates them and `just generated-check` fails on drift.
