...
use flower_conformance::{
    CompileExpectation, ConformanceFixture, ExpectedEngineError, ExpectedPlan,
    TransitionExpectation, TransitionStep,
};
use flower_kernel::{
...
fn main() {
    let output = env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .expect("usage: generate_retry_v0_2 <output-file>");
    ...
    .unwrap();
    let first_timer_fired = timer_event(&first_wait.snapshot, "event-first-timer");
    let second_attempt =
        transition(&plan, Some(&first_wait.snapshot), first_timer_fired.clone()).unwrap();
    let second_failure = failure_event(
        &second_attempt.snapshot,
        "event-second-failure",
        retryable_code.clone(),
    );
    ...
    .unwrap();
    let second_timer_fired = timer_event(&second_wait.snapshot, "event-second-timer");
    ...
    .unwrap();
    let exhausted_failure = failure_event(
        &third_attempt.snapshot,
        "event-exhausted-failure",
        retryable_code,
    );
    ...
    let fixture = ConformanceFixture {
        schema_version: "flower.conformance/v0.2".to_owned(),
...
fn timer_event(snapshot: &ExecutionSnapshot, event_id: &str) -> ExecutionEvent {
...
fn step(
    plan: &flower_plan::ExecutableWorkflowPlan,
