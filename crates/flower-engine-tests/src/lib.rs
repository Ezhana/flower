//! Shared regression fixtures for the Rust engine and its Component binding.

use flower_engine::{
    Diagnostic, ExecutableWorkflowPlan, ExecutionEffect, ExecutionEvent, ExecutionSnapshot,
    PlanFingerprint, PlanNode, Transition, WorkflowDefinition, WorkflowId,
};
use serde::{Deserialize, Serialize};

pub const FIXTURE_SCHEMA_VERSION: &str = "flower.engine-fixture/v1";
...
mod tests {
    ...
    use flower_engine::{compile, transition};

    use super::*;

    #[test]
    fn every_engine_fixture_executes_its_declared_expectations() {
        let fixture_directory = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../fixtures");
        serde_json::from_slice::<serde_json::Value>(
            &fs::read(fixture_directory.join("fixture-schema.json"))
                .expect("read fixture JSON Schema"),
        )
        .expect("fixture JSON Schema is valid JSON");
        let directory = fixture_directory.join("cases");
        let mut paths = fs::read_dir(directory)
            ...
            .collect::<Vec<_>>();
        paths.sort();
        assert!(!paths.is_empty(), "engine fixture suite has no fixtures");

        let mut names = BTreeSet::new();
        for path in paths {
            let fixture: EngineFixture = serde_json::from_slice(
    ...
    fn tampering_any_expected_fixture_value_is_detected() {
        ...
        for path in paths {
            ...
            for expected_path in expected_paths {
                ...
                let detected = serde_json::from_value::<EngineFixture>(tampered)
    ...
    fn fixture_matches(fixture: &EngineFixture) -> bool {
        match (
